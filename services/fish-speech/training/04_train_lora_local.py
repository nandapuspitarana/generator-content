...
def run_local_lora_training(max_steps: int = 100, val_interval: int = 25, project_name: str = "indonesia-tts-rtx5060", resume: bool = False):
    ...
    ckpt_dir = fish_repo_dir / "results" / project_name / "checkpoints"
    ...
    # Clean previous checkpoint files safely (do not rmtree directory to avoid exFAT lock)
    if not resume and ckpt_dir.exists():
        for old_file in ckpt_dir.glob("*.ckpt"):
            try:
                old_file.unlink()
                print(f"🧹 Removed previous checkpoint: {old_file.name}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--project", type=str, default="indonesia-tts-rtx5060", help="Project output name")
    parser.add_argument("--resume", action="store_true", help="Resume from latest checkpoint instead of starting clean")
    args = parser.parse_args()

    run_local_lora_training(args.max_steps, args.val_interval, args.project, args.resume)
